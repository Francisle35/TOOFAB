...
%      applied currentsi from time 0 to tf, an array which 
...
%
% Author: Francis le Roux & Zuan Khalik (f.a.l.le.roux@tue.nl)
%
% TOOFAB is licensed under the BSD 3-Clause License
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function out = DFN_Ageing_Cycle(input_current,final_time,init_cond,varargin)
...
j_tot = zeros(p.nnp,min(max_prealloc_size,round(time_max/p.dt)));
eta_main = zeros(p.nnp,min(max_prealloc_size,round(time_max/p.dt)));
...
eta_sei = zeros(p.nnp,min(max_prealloc_size,round(time_max/p.dt)));
j_sei = zeros(p.nnp,min(max_prealloc_size,round(time_max/p.dt)));

% ~~~~~~~~~~~ Ageing Variables ~~~~~~~~~~~ %
epse_neg_temp = zeros(p.nnp,min(max_prealloc_size,round(time_max/p.dt)));
epse_neg_temp(:,1) = p.epse_neg;
j_lpl = zeros(p.nnp,min(max_prealloc_size,round(time_max/p.dt)));
j_cath = zeros(p.nnp,min(max_prealloc_size,round(time_max/p.dt)));
A_max = p.A_max; % 0.05; % Estimated from Given Particle size and Fig. 4 Barai et al.
A_cr = zeros(1,round(time_max/p.dt));
Ds_neg_crack = zeros(1,round(time_max/p.dt));
Ds_neg = p.Ds_neg;
epss_neg_temp = zeros(p.nn,min(max_prealloc_size,round(time_max/p.dt))); 
epss_neg_temp(:,1) = p.epss_neg;
sigma_neg = zeros(p.nn,min(max_prealloc_size,round(time_max/p.dt))); 
sigma_neg(:,1) = p.sigma_eff_neg';
a_s_neg = zeros(p.nn,min(max_prealloc_size,round(time_max/p.dt))); 
a_s_neg(:,1) = p.a_s_neg;
epss_pos_temp = zeros(p.nn,min(max_prealloc_size,round(time_max/p.dt))); 
epss_pos_temp(:,1) = p.epss_pos;
a_s_pos = zeros(p.np,min(max_prealloc_size,round(time_max/p.dt)));
a_s_pos(:,1) = p.a_s_pos;
sigma_pos = zeros(p.np,min(max_prealloc_size,round(time_max/p.dt)));
sigma_pos(:,1) = p.sigma_eff_pos';
if p.ageing
...
% ~~~~~~~~~~~ Memory Variables ~~~~~~~~~~~ %
mem.CycleNo = 1; % Initialize The Cycle NUmber for CC-CV Charging 
...
%     elseif not(max_iterations) && not(dt_or==p.dt) && kl2==0 
    ...
    for k=1:p.iter_max
        ...
        [F_phis,cs(:,t+1),ce(:,t+1),phie(:,t+1),j_tot(:,t+1),j_sei(:,t+1),j_lpl(:,t+1),j_cath(:,t+1),i0(:,t+1),eta_main(:,t+1),eta_sei(:,t+1),eta_lpl(:,t+1),eta_cath(:,t+1),U(:,t+1),p,m] = fcn_F(phis_prev,ce_prevt,cs_prevt,T_prevt,i_app(t+1),p,m);
       ...
       % If algorithm doesn't converge, then display a warning
        ...
        [J_phis,p] = fcn_J(cs(:,t+1),ce(:,t+1),j_tot(:,t+1),j_tot(:,t+1),i0(:,t+1),eta_main(:,t+1),eta_sei(:,t+1),eta_lpl(:,t+1),eta_cath(:,t+1),T_prevt,p,m); 
    ...
    % ~~~~~~~~~~~ Compute Ageing Parameters ~~~~~~~~~~~ % 
    if p.ageing   
        % Compute Main Flux (At Negative Electrode)
        j_main(:,t) = j_tot(:,t)-j_sei(:,t)-j_lpl(:,t)-j_cath(:,t);
        
        if p.ageMech.PoreClog
        % ~~~ Pore Clogging ~~~ %

        if epse_neg_temp(:,t) > 0.001 % NOTE: Added if condition such that epse_neg does not pass below threshold to avoid break
           epse_neg_temp(:,t+1) = epse_neg_temp(:,t) - (p.dt*p.beta_pc_sei*j_sei(:,t+1)*p.F + p.dt*p.beta_pc_lpl*j_lpl(:,t+1)*p.F); % <-- Added the effect of Lithium Plating on Pore Clogging
        else
            epse_neg_temp(:,t+1) = 0.001;
        end

        end
        

        if p.ageMech.Cathode
        % ~~~ Cathode Degradation ~~~ %

        % Change in porosity due to dissolution reaction
        beta_cath = p.beta_cath;
        epss_pos_temp(:,t+1) = epss_pos_temp(:,t) + p.dt*p.beta_cath*p.F*j_cath(p.nn+1:end,t+1);

        % Lost Charge
        Closs_cath(t+1) = Closs_cath(t) + p.F*p.dt*p.dx_p*p.A_surf*(sum(abs(j_cath(1+p.nn:end,t+1).*p.a_s(1+p.nn:end))));
        else
        Closs_cath(t+1) = 0;
        end

        if p.ageMech.SurfaceCrack
        % ~~~ Surface Cracking ~~~ %
        beta_5 = p.beta_sc2; 
        beta_4 = p.beta_sc1;

        Ds_neg_crack(t) = Ds_neg*(1 - A_cr(t)/A_max)^beta_5;
        A_cr(t+1) = A_cr(t) + p.dt*beta_4*(A_max - A_cr(t))*abs(i_app(t));
        end

        if p.ageMech.LAM
        % ~~~ Loss of Active Material ~~~ %
        C_1 = p.lam_1; %2.68e-6 [Delacourt et. Al, 2012]
        C_2 = p.lam_2; %-1.3e-4

        % Change in volume fraction due to LAM
        epss_neg_temp(:,t+1) = epss_neg_temp(:,t) - p.dt*C_1*abs(j_main(1:p.nn,t)) + p.dt*C_2*sqrt(abs(j_main(1:p.nn,t))); % Eq. 5 Safari and Delacourt // Eq. 26 Reniers et al
        val1=p.dt*(C_1*abs(j_main(1:p.nn,t)));
        val2=p.dt*C_2*sqrt(abs(j_main(1:p.nn,t)));
        val3=val1+val2;
        sigma_neg(:,t) = p.sigma_neg.*epss_neg_temp(:,t);
        a_s_neg(:,t) = 3*epss_neg_temp(:,t)/p.R_neg;

        % Charge lost due to trapped lithium in LAM
        Closs_LAM(t+1) = Closs_LAM(t) - p.F*p.dt*((C_1*sum(abs(j_main(1:p.nn,t)))) + C_2*sum(sqrt(abs(j_main(1:p.nn,t)))))*soc(t)*(4/3*pi*p.R_neg^3)*p.cs_max_neg;
        else
        Closs_LAM(t+1) = 0;
        end 

        if p.ageMech.LiPlating
            % ~~~ Lithium Plating ~~~ %
            Closs_LPL(t+1) = Closs_LPL(t) + p.F*p.dt*p.dx_n*p.A_surf*(sum(abs(j_lpl(1:p.nn,t+1).*p.a_s(1:p.nn))));
        else
            Closs_LPL(t+1) = 0;
        end
        ...
        if p.ageMech.SEI
            % ~~~ SEI Formation ~~~ %
            Closs_SEI(t+1) = Closs_SEI(t) + p.F*p.dt*p.dx_n*p.A_surf*(sum(abs(j_sei(1:p.nn,t+1).*p.a_s(1:p.nn))));
        else
            Closs_SEI(t+1) = 0;
        end

        % ~~~ Compute Charge Lost ~~~ %   
        Cl(t+1) = Closs_SEI(t+1) + Closs_LAM(t+1) + Closs_LPL(t+1) + Closs_cath(t+1); % Combining Ageing Losses
        
        K_lpl = 0.1356; % K_lpl value to scale the effect of LPL on R_f
        Rf(:,t+1) = Rf_prevt-p.dt*p.B1.*j_sei(:,t+1)-K_lpl*p.dt*(p.V_SEI./(p.sigma_SEI).*j_lpl(:,t+1));      % Took same values as for SEI, since no values from Literature to go off of

        mem.Closs(t+1) = Cl(t+1);
    
    end
    % ~~~~~~~~~~~ Update Ageing Parameters ~~~~~~~~~~~ %
    if mod(t,500)==0 && p.ageing==1

        % ~~~ Updating New Ageing Parameters ~~~ %

        if p.ageMech.LAM
            p.epss_neg = epss_neg_temp(:,t);        % LAM affects porosity
            p.sigma_eff_neg = p.sigma_neg.*p.epss_neg;
            p.sigma_eff(1:p.nn) = p.sigma_eff_neg;  % Update Conductivity with new Epsilon
            p.a_s_neg = 3*p.epss_neg/p.R_neg;                                              
            p.a_s(1:p.nn) = p.a_s_neg;              % Update interfacial surface areaL
            p.eps_s(1:p.nn) = p.epss_neg;
        end

        if p.ageMech.PoreClog
            p.eps_e(1:p.nn) = epse_neg_temp(1:p.nn,t);     % Pore Clogging affects porosity
        end
        
        if p.ageMech.SurfaceCrack
            p.Ds_neg = Ds_neg_crack(t);          % Surface Cracking affects Diffusivity
        end

        if p.ageMech.Cathode
            p.epss_pos = epss_pos_temp(:,t);    % Cathode Degradation affects porosity
        ...
        % NEW: Update the newly time dependent parameters (p.eps_e changes with time, therefore De_eff must be updated).
        ...
        % Compute new ageing parameters
        [Cbat_new,sbat,gamma_neg, gamma_pos] = fcn_Q(Cl(t+1),se_init,1,p); 
        ...
        p.gamma_neg_prev = gamma_neg;
        p.gamma_pos_prev = gamma_pos;
        p.epss_neg_prev = p.epss_neg;
        p.epss_pos_prev = p.epss_pos;
    end
    ...
    if p.thermal_dynamics
        T(t+1) = fcn_T(j_tot(:,t+1), U(:,t+1),stoich,V(t+1),i_app(t+1),T_prevt, p);
    else
        T(t+1) = p.T_amb; 
    end
...
% Adding Time Skip Variable
Tskip = p.SaveDataTskip;
...
out.t = t_vec(2:1:n_t); 
%out.cs = cs(:,2:Tskip:n_t);
%out.ce = ce(:,2:Tskip:n_t); 
%out.phis = [phis(1:p.nn,2:Tskip:n_t); phis(p.nn+1:end,2:Tskip:n_t)]; 
%out.phie = phie(:,2:Tskip:n_t); 
% if p.set_simp(6)
%     out.cs_bar = [cs(p.nnp+(1:p.nn),2:Tskip:n_t); cs(p.nnp+(p.nn+1:p.nnp),2:Tskip:n_t)];
% else
%     out.cs_bar = [cs(p.nrn:p.nrn:p.nrn*p.nn,2:Tskip:n_t);cs(p.nrn*p.nn+p.nrp:p.nrp:end,2:Tskip:n_t)];
% end
% out.stoich = [out.cs_bar(1:p.nn,:)/p.cs_max_neg; NaN(p.ns,n_t-1); out.cs_bar(p.nns+1:end,:)/p.cs_max_pos]; 
% out.j_tot = [j_tot(1:p.nn,2:Tskip:n_t); j_tot(p.nn+1:end,2:Tskip:n_t)];  
% out.U = [U(1:p.nn,2:Tskip:n_t); U(p.nn+1:end,2:Tskip:n_t)];  
%out.eta_main = [eta_main(1:p.nn,2:Tskip:n_t);  eta_main(p.nn+1:end,2:Tskip:n_t)]; 
out.V = V(2:1:n_t); 
out.i_app = i_app(2:n_t);
% out.T = T(2:n_t); 
%out.soc = soc(2:Tskip:n_t);
par_or = p;
out.param = par_or; 
out.mem = mem;

% Adding More Outputs to Save
% out.j_main = j_main(:,2:Tskip:n_t-1);  
...
if p.ageing
    ...
    out.ageing.Closs = Cl(2:Tskip:n_t); 
    % out.ageing.Rf = [Rf(1:p.nn,end); NaN(p.ns,1); Rf(p.nn+1:end,end)];
    %out.ageing.j_sei = [j_sei(1:p.nn,2:Tskip:n_t); j_sei(p.nn+1:end,2:Tskip:n_t)]; 
    %out.ageing.eta_sei = [eta_sei(1:p.nn,2:Tskip:n_t); eta_sei(p.nn+1:end,2:Tskip:n_t)]; 
    % out.states_end.Closs = Cl(n_t); 
    % out.states_end.Rf = out.ageing.Rf; 
    
    % Adding Outputs here 
    %out.ageing.epse_neg = epse_neg_temp(:,2:Tskip:n_t-1);
    %out.ageing.j_lpl = [j_lpl(1:p.nn,2:Tskip:n_t-1);j_lpl(p.nn+1:end,2:Tskip:n_t-1)];
    %out.ageing.eta_lpl = [eta_lpl(1:p.nn,2:Tskip:n_t-1); eta_lpl(p.nn+1:end,2:Tskip:n_t-1)];
    %out.ageing.Ds_eff_neg = Ds_neg_crack(:,2:Tskip:n_t-1);
    %out.ageing.A_cr = A_cr(:,2:Tskip:n_t-1);
    %out.ageing.epss_neg = epss_neg_temp(:,2:Tskip:n_t-1);
    %out.ageing.sigma_neg = sigma_neg(:,2:Tskip:n_t-1);
    %out.ageing.a_s_neg = a_s_neg(:,2:Tskip:n_t-1);
    %out.ageing.Closs_LAM = Closs_LAM(:,2:Tskip:n_t-1);
    %out.ageing.Closs_cath = Closs_cath(:,2:Tskip:n_t-1);
    %out.ageing.j_cath = [j_cath(1:p.nn,2:Tskip:n_t-1);j_cath(p.nn+1:end,2:Tskip:n_t-1)];
    %out.ageing.eta_cath = [eta_cath(1:p.nn,2:Tskip:n_t-1); eta_cath(p.nn+1:end,2:Tskip:n_t-1)];
    % out.ageing.epss_pos = epss_pos_temp(:,2:Tskip:n_t-1);
    %out.ageing.a_s_pos = a_s_pos(:,2:Tskip:n_t-1);
    %out.ageing.sigma_pos = sigma_pos(:,2:Tskip:n_t-1);
end
...
function [F_phis,cs,ce,phie,j_tot,j_sei,j_lpl,j_cath,i0,eta_main,eta_sei,eta_lpl,eta_cath,U,p,m] = fcn_F(phis,ce_prevt,cs_prevt,T,i_app,p,m)
%-------------------------------------------------------------------------%
%- Compute F_phis and J_phis ---------------------------------------------%
%-------------------------------------------------------------------------% 
j_tot = -m.Bphis_inv*(m.Aphis*phis+m.Cphis*i_app);
...
if p.ageing
    if p.ageMech.SEI
        eta_sei = phis-phie_bar-p.Rf.*p.F.*j_tot;
        j_sei = -(p.i02).*exp(-p.alpha_SEI*p.F/(p.R*T)*eta_sei);
        j_sei(p.nn+1:end) = zeros(p.np,1);
    else
        j_sei = zeros(p.nn+p.np,1);
        eta_sei = zeros(p.nn+p.np,1);
    end

    if p.ageMech.LiPlating
        eta_lpl = phis-phie_bar-p.Rf.*p.F.*j_tot;
        j_lpl = -(p.i0lpl).*exp(-p.alpha_lpl*p.F/(p.R*T)*eta_lpl);   
        j_lpl(p.nn+1:end) = zeros(p.np,1);  % Li Plating only occurs at Anode
    else
        j_lpl = zeros(p.nn+p.np,1);
        eta_lpl = zeros(p.nn+p.np,1);
    end
        
    if p.ageMech.Cathode
        
        eta_cath = phis-phie_bar;
        j_cath = (p.i0cath).*exp(p.alpha_a*p.F/(p.R*T)*eta_cath);
        j_cath(1:p.nn,:) = zeros(p.nn,1); % Cathode degradation only occurs at cathode
        eta_cath(1:p.nn,:) = zeros(p.nn,1); 
    else
        j_cath = zeros(p.nn+p.np,1);
        eta_cath = zeros(p.nn+p.np,1);
    end

end

j_main = j_tot-j_sei-j_lpl+j_cath;


if p.set_simp(6)==1
    cs = -m.Acs_hat_inv*(m.Bcs_hat*j_main+[cs_prevt(1:p.nnp);zeros(p.nnp,1)]); 
else
    cs = -m.Acs_hat_inv*(m.Bcs_hat*j_main+cs_prevt); 
end
ce_bar = m.Ace_bar*ce; 
cs_bar = m.Acs_bar*cs; 

% Pore Clogging: Update Matrices due ot change in p.eps_e
...
eta_main = phis-phie_bar-U-p.F*p.Rf.*j_tot;


if p.set_simp(5)==1 
    F_phis = p.F*j_main./i0-(p.alpha_a+p.alpha_c)*p.F/(p.R*T)*eta_main;
else
    exp1 = exp(p.alpha_a*p.F*eta_main/(p.R*T)); 
    exp2 = exp(-p.alpha_c*p.F*eta_main/(p.R*T)); 
    F_phis = p.F*j_tot./i0-(exp1-exp2);
end
...
function [J_phis,p] = fcn_J(cs,ce,j_tot,j_main,i0,eta_main,eta_sei,eta_lpl,eta_cath,T,p,m)
%-------------------------------------------------------------------------%
%- Compute F_phis and J_phis ---------------------------------------------%
%-------------------------------------------------------------------------% 
...
dj_totdphis = -m.Bphis_inv*m.Aphis; 

dphiedphis = m.Phi_phie_bar+m.Pi_phie_bar*diag(1./ce)*m.Phi_ce;  

if p.ageing                                              
    if p.ageMech.SEI
        deta_seidphis = eye(p.nnp)-dphiedphis-diag(p.F*p.Rf)*dj_totdphis;
        dj_seidphis = diag(p.F*p.i02*2*p.alpha_SEI/(p.R*T)*exp(-2*p.alpha_SEI*p.F/(p.R*T)*eta_sei))*deta_seidphis;
        dj_seidphis(p.nn+1:end,:) = zeros(p.np,p.nnp); 
    else
        dj_seidphis = zeros(p.nnp,p.nnp);
    end

    if p.ageMech.LiPlating 
        deta_lpldphis = eye(p.nnp)-dphiedphis-diag(p.F*p.Rf)*dj_totdphis;
        dj_lpldphis = diag(p.F*p.i0lpl*2*p.alpha_lpl/(p.R*T)*exp(-2*p.alpha_lpl*p.F/(p.R*T)*eta_lpl))*deta_lpldphis;
        dj_lpldphis(p.nn+1:end,:) = zeros(p.np,p.nnp);
    else
        dj_lpldphis = zeros(p.nnp,p.nnp);
    end

    if p.ageMech.Cathode
        deta_cathdphis = eye(p.nnp)-dphiedphis;
        dj_cathdphis = diag(p.F*p.i0cath*2*p.alpha_a/(p.R*T)*exp(2*p.alpha_a*p.F/(p.R*T)*eta_cath))*deta_cathdphis;
        dj_cathdphis(1:p.nn,:) = zeros(p.nn,p.nnp);
    else
        dj_cathdphis= zeros(p.nnp,p.nnp);
    end

     dj_maindphis = dj_totdphis-dj_seidphis-dj_lpldphis+dj_cathdphis;
else
    dj_maindphis = dj_totdphis; 
end

if p.ageing
    dcsdphis = -m.Acs_j_main*dj_maindphis;
else
...
deta_maindphis = eye(p.nnp)-dphiedphis-dUdphis-diag(p.F*p.Rf)*dj_totdphis; 

if p.set_simp(5)==1 
    J_phis = p.F*diag(1./i0)*dj_maindphis-p.F*diag(j_main./i0.^2)*di0dphis-(p.alpha_a+p.alpha_c)*p.F/(p.R*T)*deta_maindphis; 
else
    exp1 = exp(p.alpha_a*p.F*eta_main/(p.R*T)); 
    exp2 = exp(-p.alpha_c*p.F*eta_main/(p.R*T)); 
    dexp1dphis = diag(p.alpha_a*p.F/(p.R*T)*exp1)*deta_maindphis; 
    dexp2dphis = -diag(p.alpha_c*p.F/(p.R*T)*exp2)*deta_maindphis; 
    J_phis = p.F*diag(1./i0)*dj_totdphis-p.F*diag(j_tot./i0.^2)*di0dphis-(dexp1dphis-dexp2dphis); % <--- Do we need to incorperate changes here too ??
end
end

function [T] = fcn_T(j_tot, U,stoich,V,i_app,T_prevt, p)
qconvec = p.hc*(p.T_amb-T_prevt); 
DeltaT = U-T_prevt*[p.dU_dT_neg(stoich(1:p.nn)); p.dU_dT_pos(stoich(p.nn+1:end))] ; 
qchem = i_app*V-p.A_surf*p.F*ones(1,p.nnp)*(p.dx(p.elec_range).*p.a_s.*j_tot.*DeltaT);  %Resistive and reversible heat generation, only heat mixing due to nonuniform current distribution is accounted for
...
if isstruct(init_cond)
    ...
    if length(init_cond)>1
        ...
        j_sei_avg_tot = Closs/(p.F*p.dt*p.dx_n*p.A_surf*p.a_s_neg*p.nn); 
        Rf = p.Rf+(p.dt*p.V_SEI./p.sigma_SEI.*j_sei_avg_tot)*[ones(p.nn,1); zeros(p.np,1)]; 
    else
        Closs = p.Closs_init;
        Rf = p.Rf; 
    end
...
function [Q, stoich_Q, gamma_neg, gamma_pos] = fcn_Q(Closs,stoich_Q_prev,indicator,p)
x_prev = stoich_Q_prev; 
% Condition to check if LAM modelled, this affeects how gamma_neg is computed (since eps_s becomes spatially dependent instead of a scalar)
if p.ageMech.LAM
    gamma_neg = p.gamma_neg_prev + (((sum(p.eps_s(1:p.nn))-sum(p.epss_neg_prev))*p.dx_n)*p.cs_max_neg*p.A_surf*p.F);
else
    gamma_neg = mean(p.epss_neg)*p.delta_neg*p.cs_max_neg*p.A_surf*p.F;
end

% Same condition checked for Cath (since eps_s becomes spatially dependent with cath)
if p.ageMech.Cathode
    gamma_pos = p.gamma_pos_prev + (((sum(p.eps_s(p.nn+1:end))-sum(p.epss_pos_prev))*p.dx_p)*p.cs_max_pos*p.A_surf*p.F);
else
...
stoich_Q = x; 
Q = (stoich_Q(1)-stoich_Q(3))*gamma_neg;
end
...
Acs_j_tot = sparse(fcn_Acs_j(p.nrn));
m.Acs_jp = sparse(fcn_Acs_j(p.nrp));
Acs_n = sparse(kron(eye(p.nn),Acs_j_tot)*diag((1/p.dr_n^2)*p.Ds_neg*ones(p.nn,1)'*kron(eye(p.nn), ones(1,p.nrn))));
m.Acs_hat_n = p.dt*Acs_n-speye(p.nrn*p.nn);
m.Acs_hat_inv_n = inv(m.Acs_hat_n); 

Bcs_j_tot = -2*(p.dr_n+p.R_neg)/(p.dr_n*p.R_neg)*[zeros(p.nrn-1,1); 1];
Bcs_jp = -2*(p.dr_p+p.R_pos)/(p.dr_p*p.R_pos)*[zeros(p.nrp-1,1); 1];
Bcs_n = sparse(kron(eye(p.nn), Bcs_j_tot));
Bcs_p = sparse(kron(eye(p.np), Bcs_jp));
Bcs = blkdiag(Bcs_n,Bcs_p);
m.Bcs_hat = Bcs*p.dt; 
end

% NOTE: here is a candidate spot to change p.epse_e PC
Bce = (diag(p.a_s.*(1-p.t_plus)./(p.eps_e(p.elec_range)))); 
Bce = sparse([Bce(1:p.nn,:); zeros(p.ns,p.nnp); Bce(p.nn+1:end,:)]);
m.Bce_hat = p.dt*Bce; 
...
function m = Ds_matrices(stoich,T,p,m)
    if p.set_simp(6)
        p.Ds = [p.Ds_neg*ones(p.nn,1); ones(p.np,1).*p.Ds_pos(stoich,T)]; 
        ...
        % Surface Cracking Calls for an Update of p.Ds_neg
        Acs_j_tot = sparse(fcn_Acs_j(p.nrn));
        Acs_n = sparse(kron(eye(p.nn),Acs_j_tot)*diag((1/p.dr_n^2)*p.Ds_neg*ones(p.nn,1)'*kron(eye(p.nn), ones(1,p.nrn))));
        m.Acs_hat_n = p.dt*Acs_n-speye(p.nrn*p.nn);
        m.Acs_hat_inv_n = inv(m.Acs_hat_n);
    ...
    m.Acs_j_main = m.Acs_bar*m.Acs_hat_inv*m.Bcs_hat; 
end
...
p.De_eff = @(c,T) p.De(c,T).*p.eps_e.^p.brug; % <--- NOTE: Electrolyte Phase Diffusivity D_e^eff = D_e*eps_e^p
...
p.Vmin = 2; p.Vmax = 6; %minimum and maximum allowable voltages. If these bounds are exceeded, the simulation stops
...
p.T_amb = 293.15; %ambient temperature
...
p.i02 = 1.178e-7;% @(T) 10*(1.34368000000001e-11*T.^2-7.29948384000006e-09*T+9.9367796744801e-07); % [A / m^2] Exchange current density of the side reactions
p.alpha_SEI = 0.7;
p.V_SEI = 2e-6; % Molar volume of the SEI
p.sigma_SEI = 2.3e-6; %Conductivity of the SEI
p.Cbat_init = 0;

% NEW: Added ageing parameters
p.alpha_lpl = 0.5; 
p.i0lpl = 0.001; % Exchange current density of the Li Plating reactions
p.Ucath = 4.1; % 4 [V] Kindermann et al. 2017 // 3.76 [V] Jana et al. 2019 // Lin et al. 2013 say 4.1 [V]
p.i0cath = 6.05e-6; % 6.05e-6 [A / m^2] Kindermann et al. 2017 // 6 [A] (Cycling) - 7e-6 [A] (storage)  Jana et al. 2019 
end 
